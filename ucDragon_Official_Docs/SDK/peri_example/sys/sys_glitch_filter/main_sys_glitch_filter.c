...
static void app_setup_pin(void)
{
	...
	Chip_SWM_MovablePinAssign(SWM_PWM_OUT, LED1_PIN);
...
int main(void)
{
	...
	Board_LED_Set(0, false);	/* shut off LED 0 */
	Board_LED_Set(1, false);	/* shut off LED 1*/
	...
	Chip_IOCON_PinSetClockDivisor(LPC_IOCON, LED1_IOCON, IOCONCLKDIV0);	/* assign IOCON clock divider 0 to RED LED pin */
	Chip_IOCON_PinSetSampleMode(LPC_IOCON, LED1_IOCON, PIN_SMODE_CYC1);	/* enable the 1 cycle glitch mode */

	/* Initialize pin interrupt */
	Chip_SYSCTL_SetPinInterrupt(0, LED1_PIN);							/* Set pin interrupt 0 to RED LED pin*/
