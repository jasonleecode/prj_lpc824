...
#define GPIO_PININT		1
#define GPIO_PININT2	4
#else
...
/**
 * @brief	Handle interrupt from PININT2
 * @return	Nothing
 */
...
int main(void)
{
	...
	Chip_SWM_MovablePinAssign(SWM_SCT_IN2_I, GPIO_PININT2);

	/* Configure channel 2 interrupt as edge sensitive and falling edge interrupt */
	Chip_PININT_SetPinModeEdge(LPC_PININT, PININTCH2);
	Chip_PININT_EnableIntLow(LPC_PININT, PININTCH2);

	/* Configure interrupt channel 2 for the GPIO pin in SysCon block */
	Chip_SYSCTL_SetPinInterrupt(2, GPIO_PININT2);

	/* Configure channel 2 as wake up interrupt in SysCon block */
	Chip_SYSCTL_EnablePINTWakeup(2);

	/* Configure GPIO pin as input pin */
	Chip_GPIO_PinSetDIR(LPC_GPIO_PORT, 0, GPIO_PININT2, 1);
