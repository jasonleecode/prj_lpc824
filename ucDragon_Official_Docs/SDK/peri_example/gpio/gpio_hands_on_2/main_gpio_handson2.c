...
#define KEY1_PIN		4 /*wakeup key*/
#define KEY2_PIN		1 /**user key/
...
int main(void) {
	...
	uint32_t KEY_GPIO_CONFIG = ( (1 << KEY1_PIN) | (1 << KEY2_PIN));
	...
	while (1) {
		...
		if( Chip_GPIO_PinGetState(LPC_GPIO_PORT, 0, KEY1_PIN) == 0 ) {
			...
			// key1 button
		if( Chip_GPIO_PinGetState(LPC_GPIO_PORT, 0, KEY2_PIN) == 0 ) {
			Chip_GPIO_PortSetOutLow(LPC_GPIO_PORT, 0, (1 << LED2_PIN));
		}
		else {
			Chip_GPIO_PortSetOutHigh(LPC_GPIO_PORT, 0, (1 << LED2_PIN));
		}
	}
}
